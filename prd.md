 ...
 **bug或细节优化**
    ...
    - ✅ 在gallery页点赞后，跳转到其他页面，再回到gallery页，刚刚点赞过的图片可以再次点赞（已使用localStorage持久化）

 **运营手段与SEO辅助**
    - ✅ 对Charlie XcX的热门歌曲的歌词生成对应的图片，期望通过图片alt获取搜索流量（已创建 /admin/lyrics 管理页面）

 **性能优化**
   - ✅ Gallery加载速度优化。目前跳转到gallery时，会有半秒左右的"loading"提示。（已使用SSR+ISR优化）
   - ✅ 首屏加载速度优化（字体优化、DNS预解析、CSS内联、组件懒加载、Gzip压缩等）
   - ✅ 代码分割和懒加载，这个是什么？先跟我解释清楚，我认为需要做时，才开始开发（已解释，目前优化已充分）
   - ✅ Gallery点赞优化，目前gallery点赞之后，需要等待1-2秒才点赞成功（已使用optimistic UI）
